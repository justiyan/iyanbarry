    ...
    <Layout>
      ...
      <Shell>
        <section className="home-block" id="writing">
          ...
          <div>{posts.map(post => <Link href={`/blog/${post.slug}`} className="home-article" key={post.slug}><div className="post-date pt-[7px]"><PostDate date={post.date} updated={post.updated} retrospectiveDate={post.retrospectiveDate} /></div><div><h3>{post.title}</h3><p>{post.summary}</p><p className="mt-s2 text-[12px]">{post.readingMinutes} min read</p></div><span className="text-[24px] text-accent" aria-hidden="true">↗</span></Link>)}</div>
        </section>
